---
# No `title:` — the hero logo leads the page, so we skip the title block entirely.
pagetitle: "Welcome to SEMinR"
toc: false
---

```{=html}
<!-- Homepage only: tag the pinned header so the navbar starts large and shrinks
     to the normal size on scroll (styled in custom.scss via headroom's --top
     state). Only present on this page, so the hero navbar is homepage-scoped. -->
<script>document.getElementById("quarto-header")?.classList.add("nav-hero");</script>
```
