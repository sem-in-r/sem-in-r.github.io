...
Like SEMinR itself, seminrExtras is **R-first**: the R package is the
authoritative implementation, and the Python and JavaScript/TypeScript ports
maintain **numerical parity** with it (validated against R-generated golden
fixtures). Each port consumes an already-estimated model from the
matching-language SEMinR port, so it comes in the same **three variants**:
...
[[![seminrExtras on CRAN](https://img.shields.io/cran/v/seminrExtras.svg?label=CRAN&color=2166ac&style=flat-square)](https://cran.r-project.org/package=seminrExtras)]{.pkg-badge}
...
[[![seminrExtras on PyPI](https://img.shields.io/pypi/v/seminrExtras.svg?label=PyPI&color=2166ac&style=flat-square)](https://pypi.org/project/seminrExtras/)]{.pkg-badge}

A Python port with numerical parity to the R package, built on the
[`seminr`](https://pypi.org/project/seminr/) estimation engine. Requires
Python 3.11+.

> **Experimental.** Please report any issues on its
> [GitHub repository](https://github.com/sem-in-r/seminrExtras-py/issues).

**Install from PyPI** (pulls in `seminr` automatically):

```bash
pip install seminrExtras
```

**Optional extras** — matplotlib plots and pandas-friendly input:

```bash
pip install "seminrExtras[plot]"
pip install "seminrExtras[pandas]"
```

Then evaluate an already-estimated `seminr` model:
...
- **Source:** <https://github.com/sem-in-r/seminrExtras-py>
- **PyPI:** <https://pypi.org/project/seminrExtras/>

### <img src="images/logos/typescript.svg" alt="" class="lang-logo"> JavaScript & TypeScript

[[![@seminr/extras on npm](https://img.shields.io/npm/v/@seminr/extras.svg?label=npm&color=2166ac&style=flat-square)](https://www.npmjs.com/package/@seminr/extras)]{.pkg-badge}

A from-scratch TypeScript port that consumes an already-estimated model from
[`@seminr/core`](https://www.npmjs.com/package/@seminr/core), the TypeScript
SEMinR estimation engine. Pure ESM, targeting Node 18+ / Bun.

> **Experimental.** `@seminr/extras` is in early development (`0.x`) — please report
> any issues on its [GitHub repository](https://github.com/sem-in-r/seminrExtras-ts/issues).

**Install from npm** (alongside the core estimation engine):

```bash
npm install @seminr/core @seminr/extras
```

Then assess an estimated SEMinR model:

```typescript
import { assessCvpat } from "@seminr/extras";

const result = assessCvpat(model, { nboot: 2000, seed: 123 })!;
console.log(String(result));
```

- **Source:** <https://github.com/sem-in-r/seminrExtras-ts>
- **npm:** <https://www.npmjs.com/package/@seminr/extras>
